...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db: db_dependency):
    print(f"Login attempt for username: {form_data.username}")  # Debug login attempt
    user = authenticate_user(form_data.username, form_data.password, db)
    if not user:
        print("Authentication failed")  # Debug authentication failure
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Incorrect username or password"
        )
    
    print(f"User authenticated: {user.username}")  # Debug successful authentication
